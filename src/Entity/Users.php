...
class Users implements UserInterface
{
    ...
    /**
     * @ORM\OneToMany(targetEntity=PostLike::class, mappedBy="Users")
     */
    private $likess;
    ...
    public function addLikess(PostLike $likess): self
    {
        if (!$this->likess->contains($likess)) {
            $this->likess[] = $likess;
            $likess->setUsers($this);
        }

        return $this;
    }

    public function removeLikess(PostLike $likess): self
    {
        if ($this->likess->removeElement($likess)) {
            // set the owning side to null (unless already changed)
            if ($likess->getUsers() === $this) {
                $likess->setUsers(null);
            }
        }
